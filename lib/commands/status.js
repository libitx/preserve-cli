...
module.exports = {
  ...
  displayFileList(argv, router) {
    const table = new Table({
      head: ['File', 'Size', emojic.globeWithMeridians],
      chars: {
    ...
    if (argv.show === 'all' ||
      ...
      table.push([
        ...
        { vAlign: 'bottom', content: router.exists ? emojic.whiteCheckMark : emojic.x }
      ])                
    }
    ...
    Object.keys(router.files)
      ...
      .forEach(file => {
        ...
        table.push([
          ...
          { vAlign: 'bottom', content: valid ? (router.files[file].exists ? emojic.whiteCheckMark : emojic.x) : emojic.warning }
        ])
      })
